...
#  MODIFY compute_pos_weights function
def compute_pos_weights(y_train: np.ndarray) -> torch.Tensor:
    ...
    # Return on GPU instead of CPU
    return torch.tensor(pos_weights, dtype=torch.float64, device=DEVICE)
...
class PureQuantumModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Keep dimensionality reduction on GPU
        x_reduced = self.dim_reduction(x)  # Shape: (batch_size, N_QUBITS)
    
        # Process through quantum circuit (MUST move to CPU for PennyLane)
        expectations = torch.zeros(batch_size, N_LABELS, dtype=torch.float64, device=x.device)
    
        # Move to CPU only for quantum processing, then immediately back to GPU
        x_reduced_cpu = x_reduced.detach().cpu()
    
        for i in range(batch_size):
            try:
             # Quantum circuit MUST run on CPU
                circuit_output = quantum_circuit(x_reduced_cpu[i], self.quantum_weights)
             # Immediately move result back to GPU
                expectations[i] = torch.stack(circuit_output).to(x.device, non_blocking=True)
            except Exception as e:
                logger.error(f"Quantum circuit error for sample {i}: {e}")
                expectations[i] = torch.zeros(N_LABELS, dtype=torch.float64, device=x.device)
    
        # All scaling operations stay on GPU
        logits = expectations * self.output_scales + self.output_biases
        return logits
...
def train_model(model: PureQuantumModel, train_loader: DataLoader, val_loader: DataLoader,
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, (batch_X, batch_y) in enumerate(train_pbar):
            try:
                ...
                # Update progress bar
                if batch_idx % 100 == 0:  # Update every 100 batches
    ...
    if best_model_state is not None:
        ...
        # Restore proper device placement - move all to GPU first
        model.to(DEVICE)
    
        # Then keep quantum weights on CPU
...
def main():
    ...
    try:
        ...
        # Move entire model to GPU first
        model.to(DEVICE)

        # Then explicitly keep quantum weights on CPU (PennyLane requirement)
        for i, p in enumerate(model.quantum_weights):
            model.quantum_weights[i] = nn.Parameter(p.detach().to('cpu'), requires_grad=True)
